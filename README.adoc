...
|[source,powershell]
----
17 # (warning: returns all output values that are not captured, not just the last one)
return 17 # (warning returns: all output values that are not captured, not just the last one)
----
